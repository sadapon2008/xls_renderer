...
public class XRXlsxRenderer {
    ...
    protected void setPictureSize(XSSFPicture picture, int pixcels_per_col, int pixcels_per_row, int col, int row, int num_col, int num_row) {
                    ...
                    if((params == null) || (params.length < 1) || ("string".equalsIgnoreCase(params[0]))) {
                        ...
                        try {
                            ...
                            if("image_jpg".equalsIgnoreCase(params[0])) {
                                pictureIndex = book.addPicture(bytes, XSSFWorkbook.PICTURE_TYPE_JPEG);
                            } else {
                                pictureIndex = book.addPicture(bytes, XSSFWorkbook.PICTURE_TYPE_PNG);
                            }
                            img.close();
                        } catch(IOException e) {
                            continue;
                        ...
                        // 画像を挿入する
                        XSSFCreationHelper helper = (XSSFCreationHelper) book.getCreationHelper();
